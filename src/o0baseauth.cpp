...
bool O0BaseAuth::linked() {
    QString key = O2_KEY_LINKED.arg(clientId_);
...
void O0BaseAuth::setLinked(bool v) {
    ...
    QString key = O2_KEY_LINKED.arg(clientId_);
    store_->setValue(key, v? QLatin1String("1"): QLatin1String(""));
    if (oldValue != v) {
...
QString O0BaseAuth::tokenSecret() {
    QString key = O2_KEY_TOKEN_SECRET.arg(clientId_);
...
QString O0BaseAuth::token() {
    QString key = O2_KEY_TOKEN.arg(clientId_);
    return store_->value(key);
}
...
QVariantMap O0BaseAuth::extraTokens() {
    QString key = O2_KEY_EXTRA_TOKENS.arg(clientId_);
...
QByteArray O0BaseAuth::createQueryParameters(const QList<O0RequestParameter> &parameters) {
    ...
    foreach (O0RequestParameter h, parameters) {
        ...
        ret.append(QUrl::toPercentEncoding(h.name) + "=" + QUrl::toPercentEncoding(QString::fromLatin1(h.value)));
    }
    return ret;
}
